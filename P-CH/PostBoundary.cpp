	...
	for(int k=0;k<pnum;k++){
		...
		for(int i=0;i<BoundVer[k].size();i++){
			for(int j=i+1;j<BoundVer[k].size();j++){
				ID1=BoundVer[k][i];
				ID2=BoundVer[k][j];
				wlocal=QueryCH(ID1,ID2,NeighborCons[k]);
				woverlay=QueryCH(ID1,ID2,NeighborConOverlay);
...
int Graph::QueryPost(int ID1, int ID2){
	int d=INF;
	...
	if(TotalBoundSet.find(ID1)!=TotalBoundSet.end() && TotalBoundSet.find(ID2)!=TotalBoundSet.end()){
		...
		if(VtoParID[ID1]==VtoParID[ID2]){//the same partition
			d=QueryCH(ID1,ID2,NeighborConsPost[VtoParID[ID2]]);
...
void Graph::EffiCheckPost(string filename,int runtimes){
	...
	for(int i=0;i<runtimes;i++){
		QueryPost(ODpair[i].first,ODpair[i].second);
	}
...
//cout<<"44444444444444444444444444"<<endl;
	...
	for(int i=0;i<ODpair1.size();i++){
		QueryCH(ODpair1[i].first,ODpair1[i].second,NeighborConOverlay);
	}
	t2=std::chrono::high_resolution_clock::now();
	time_span = std::chrono::duration_cast<std::chrono::duration<double>>(t2-t1);
	runT= time_span.count();
	...
	for(int i=0;i<ODpair2.size();i++){
		QueryCH(ODpair2[i].first,ODpair2[i].second,NeighborConsPost[VtoParID[ODpair2[i].first]]);
	}
	t2=std::chrono::high_resolution_clock::now();
	time_span = std::chrono::duration_cast<std::chrono::duration<double>>(t2-t1);
	runT= time_span.count();
	...
	for(int i=0;i<ODpair3.size();i++){
		QueryCHOutOut(ODpair3[i].first,ODpair3[i].second,VtoParID[ID1],VtoParID[ID2]);
	}
	t2=std::chrono::high_resolution_clock::now();
	time_span = std::chrono::duration_cast<std::chrono::duration<double>>(t2-t1);
	runT= time_span.count();
	...
	for(int i=0;i<ODpair4.size();i++){
		QueryCHInOut(ODpair4[i].first,ODpair4[i].second,VtoParID[ID1]);
	}
	t2=std::chrono::high_resolution_clock::now();
	time_span = std::chrono::duration_cast<std::chrono::duration<double>>(t2-t1);
	runT= time_span.count();
