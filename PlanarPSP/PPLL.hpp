...
//        cout<<"Batch "<<b1<<": "<<hID<<"("<<NodeOrder[hID]<<") "<<PartiTag[hID].second<<endl;
        ...
        vector<vector<tuple<int,int,int>>> labelTemps;
...
//        if(false){
            labelTemps.assign(threadnum,vector<tuple<int,int,int>>());
...
//                    thread.add_thread(new boost::thread(&Graph::BatchPCLDijk2, this, boost::ref(ProcessID[i]), boost::ref(setNodes), hID, boost::ref(Neighbor), boost::ref(Label)));
                    thread.add_thread(new boost::thread(&Graph::BatchPCLDijk2New, this, boost::ref(ProcessID[i]), boost::ref(setNodes), hID, boost::ref(Neighbor), boost::ref(Label), boost::ref(labelTemps[i])));
                }
                thread.join_all();
...
//                    thread.add_thread(new boost::thread(&Graph::BatchPCLDijk, this, ID, boost::ref(setNodes), hID, boost::ref(Neighbor), boost::ref(Label)));
                    thread.add_thread(new boost::thread(&Graph::BatchPCLDijkNew, this, ID, boost::ref(setNodes), hID, boost::ref(Neighbor), boost::ref(Label), boost::ref(labelTemps[i])));
                }
                thread.join_all();
            }

        }else{//use single thread
            labelTemps.assign(bNodes.size(),vector<tuple<int,int,int>>());
            for(int i=0;i<bNodes.size();i++){
                ID = bNodes[i];
//                BatchPCLDijk(ID, setNodes, hID, Neighbor, Label);
                BatchPCLDijkNew(ID, setNodes, hID, Neighbor, Label, labelTemps[i]);
...
//                }
        ...
        for(int i=0;i<labelTemps.size();++i){
            if(!labelTemps[i].empty()){
                for(auto it=labelTemps[i].begin();it!=labelTemps[i].end();++it){
                    Label[get<0>(*it)].insert({get<1>(*it),get<2>(*it)});
                }
            }
        }
        tt.stop();
...
//    Labels.assign(node_num,map<vertex,int>());
    ...
    cout<<"Batch size: "<<BPCLBSize/partiNum+1<<endl;
    if(ifParallel){
...
//            BPCLIndexConstructPartiMap(pid, NeighborsParti, Labels, PruningPointSetP, PruningPointSetP2);
        }
    }
}
...
void Graph::BPCLIndexConstructPartiVMap(vector<int>& p, vector<vector<pair<vertex, int>>> &Neighbor, vector<map<vertex,int>>& Label, vector<unordered_map<vertex,unordered_set<vertex>>>& PruningPointSet, vector<unordered_map<vertex,vertex>>& PruningPointSet2) {
    int pid;
    for(int i=0;i<p.size();++i){
        pid=p[i];
        BPCLIndexConstructPartiMap(pid, Neighbor, Label, PruningPointSet, PruningPointSet2);
    }
}
...
void Graph::BPCLIndexConstructPartiMap(int pid, vector<vector<pair<vertex, int>>> &Neighbor, vector<map<vertex,int>>& Label, vector<unordered_map<vertex,unordered_set<vertex>>>& PruningPointSet, vector<unordered_map<vertex,vertex>>& PruningPointSet2) {
    bool ifParallel = true;
...
//    cout<<"Step for show: "<<stepShow<<endl;
    ...
    int batchSize = 512;
    batchSize = BPCLBSize/partiNum+1;
    if(batchSize>node_num){
        batchSize=node_num;
    }
//    batchSize = 3;
...
//        if(false){
            labelTemps.assign(partiThread,vector<tuple<int,int,int>>());
            if(batchSize > partiThread){
                ...
                for(int i=0;i<ProcessID.size();i++){
//                    thread.add_thread(new boost::thread(&Graph::BatchPCLDijk2, this, boost::ref(ProcessID[i]), boost::ref(setNodes), hID, boost::ref(Neighbor), boost::ref(Label)));
                    thread.add_thread(new boost::thread(&Graph::BatchPCLDijk2NewMap, this, boost::ref(ProcessID[i]), boost::ref(setNodes), hID, boost::ref(Neighbor), boost::ref(Label), boost::ref(labelTemps[i])));
                }
                thread.join_all();
                ...
                for(int i=0;i<bNodes.size();i++){
                    ID = bNodes[i];
//                    thread.add_thread(new boost::thread(&Graph::BatchPCLDijk, this, ID, boost::ref(setNodes), hID, boost::ref(Neighbor), boost::ref(Label)));
                    thread.add_thread(new boost::thread(&Graph::BatchPCLDijkNewMap, this, ID, boost::ref(setNodes), hID, boost::ref(Neighbor), boost::ref(Label), boost::ref(labelTemps[i])));
                }
                thread.join_all();
            }
            ...
            for(int i=0;i<bNodes.size();i++){
                ID = bNodes[i];
//                BatchPCLDijk(ID, setNodes, hID, Neighbor, Label);
                BatchPCLDijkNewMap(ID, setNodes, hID, Neighbor, Label, labelTemps[i]);
//                for(int j=0;j<vp.size();j++){
//                    Label[vp[j].first].insert(make_pair(ID, vp[j].second));
//                    //cout<<vp[j].first<<" "<<vp[j].second<<endl;
//                }
...
//    cout<<"Time used for label construction: "<<tt2.GetRuntime()<<" s."<<endl;
    ...
    tt2.start();
//    cout<<"Begin to construct pruning points..."<<endl;
    if(updateType==EdgeIncrease || updateType==EdgeDeletion || updateType==VertexDeletion){
        PruningPointBuild(ifParallel, processID, Neighbor, Label, PruningPointSet, PruningPointSet2);//useless for edge weight decrease
    }
//    PruningPointBuild(false);
    tt2.stop();
//    cout<<"Time used for pruning point construction: "<<tt2.GetRuntime()<<" s."<<endl;
...
void Graph::PruningPointBuild(bool ifParallel, vector<vector<vertex>> & processID, vector<vector<pair<vertex,int>>> &Neighbor, vector<map<vertex,int>>& Label, vector<unordered_map<vertex,unordered_set<vertex>>>& PruningPointSet, vector<unordered_map<vertex,vertex>>& PruningPointSet2){
...
//        cout<<"Batch size: "<<processID[0].size()<<endl;
        boost::thread_group thread;
        for(auto j=0;j<processID.size();++j){
            thread.add_thread(new boost::thread(&Graph::PPRConstruction2Map, this, boost::ref(processID[j]), boost::ref(Neighbor), boost::ref(Label), boost::ref(PruningPointSet), boost::ref(PruningPointSet2) ));
        }
        thread.join_all();
    }else{
        for(auto j=0;j<processID.size();++j){
            PPRConstruction2Map(processID[j], Neighbor, Label, PruningPointSet, PruningPointSet2);
        }
    }
}
...
void Graph::PPRConstruction2Map(vector<vertex> & p, vector<vector<pair<vertex,int>>> &Neighbor, vector<map<vertex,int>>& Label, vector<unordered_map<vertex,unordered_set<vertex>>>& PruningPointSet, vector<unordered_map<vertex,vertex>>& PruningPointSet2){
//    cout<<"1"<<endl;
    for(int i=0;i<p.size();++i){
        PPRConstruction(p[i], Neighbor, Label, PruningPointSet, PruningPointSet2);
    }
...
//function of building the pruning point records for a given vertex, lock version
...
void Graph::PPRConstruction(vertex nodeID, vector<vector<pair<vertex,int>>> &Neighbor, vector<map<vertex,int>>& Label,  vector<unordered_map<vertex,unordered_set<vertex>>>& PruningPointSet, vector<unordered_map<vertex,vertex>>& PruningPointSet2){
    benchmark::heap<2, int, int> pqueue(node_num);
    vector<bool> closed(node_num, false);
    vector<int> distance(node_num, INF);
...
//function of pruning Dijkstra of batch PLL, 2024-08-09, no lock version, vector version
void Graph::BatchPCLDijkNew(vertex nodeID, unordered_set<vertex>& setNodes, vertex hID, vector<vector<pair<vertex,int>>> &Neighbor, vector<unordered_map<vertex,int>>& Label, vector<tuple<int,int,int>> &labelTemp){
    benchmark::heap<2, int, int> pqueue(node_num);
    pqueue.update(nodeID,0);
...
//                }
                ...
                if(TempDis>topNodeDis) {//if the peak path is longer than this vally path
                    if(NodeOrder[peak[topNodeID]] <= NodeOrder[nodeID]) {

//                        vSm[topNodeID]->wait();
//                        if (Label[topNodeID].find(nodeID) == Label[topNodeID].end()) {//if not found
//                            Label[topNodeID].insert(make_pair(nodeID, topNodeDis));
//                        }
                        labelTemp.emplace_back(topNodeID,nodeID,topNodeDis);//push manner
//                        vSm[topNodeID]->notify();
                    }
//                    else {//if the peakhub is higher than topNodeID
//                        cout<<"Skip L1 "<<nodeID<<"("<<NodeOrder[nodeID]<<"),"<<topNodeID<<"("<<NodeOrder[topNodeID]<<"): "<<peakhubs[topNodeID]<<"("<<NodeOrder[peakhubs[topNodeID]]<<")"<<endl;
...
//                }
                ...
                if(TempDis<=topNodeDis) {//if the peak path is no longer than this vally path
                    continue;
                }
...
void Graph::BatchPCLDijkNewMap(vertex nodeID, unordered_set<vertex>& setNodes, vertex hID, vector<vector<pair<vertex,int>>> &Neighbor, vector<map<vertex,int>>& Label, vector<tuple<int,int,int>> &labelTemp){
    benchmark::heap<2, int, int> pqueue(node_num);
    pqueue.update(nodeID,0);
...
void Graph::BatchPCLDijk2NewMap(vector<vertex>& p, unordered_set<vertex>& setNodes, vertex hID, vector<vector<pair<vertex,int>>> &Neighbor, vector<map<vertex,int>>& Label, vector<tuple<int,int,int>> &labelTemp){
    for(auto it=p.begin();it!=p.end();++it){
        int nodeID = *it;
        BatchPCLDijkNewMap(nodeID, setNodes, hID, Neighbor, Label, labelTemp);
    }
}
void Graph::BatchPCLDijk2New(vector<vertex>& p, unordered_set<vertex>& setNodes, vertex hID, vector<vector<pair<vertex,int>>> &Neighbor, vector<unordered_map<vertex,int>>& Label, vector<tuple<int,int,int>> &labelTemp){
    for(auto it=p.begin();it!=p.end();++it){
        int nodeID = *it;
        BatchPCLDijkNew(nodeID, setNodes, hID, Neighbor, Label, labelTemp);
    }
}
//function of pruning Dijkstra of batch PLL, 2023-06-07, optimal correct version 1
...
int Graph::ShortestDisQuery1(int ID1,int ID2,vector<int>& SupNode, int& d, vector<map<vertex,int>>& Label){
    d=INF;
    int hub, dis1, dis2;
...
int Graph::ShortestDisQuery2(int ID1,int ID2,vector<int>& SupNode, int& d, vector<map<vertex,int>>& Label){
    d=INF;
...
//function for Query processing, new
int Graph::QueryPPLLDebug(int ID1, int ID2){
    int dis=INF;
    ...
    if(PartiTag[ID1].first==PartiTag[ID2].first){//if same partition
        if(PSPStrategy==PreBoundary){
            ...
            if(PartiTag[ID1].second && PartiTag[ID2].second){//Case 1: both in overlay graph
                cout<<"Same-parti: Core-Core"<<endl;
                dis=QueryOverlayPLLDebug(ID1, ID2);
            }
            else if(PartiTag[ID1].second && !PartiTag[ID2].second){//Case 2: ID2 in partition, ID1 in core
                cout<<"Same-parti: Core-Parti"<<endl;
                dis=QueryPartiCorePLL(ID2, ID1);
        ...
        if(PartiTag[ID1].second && PartiTag[ID2].second){//Case 1: both in overlay graph
            ...
            cout<<"Different Parti: Parti-Parti"<<endl;
            dis = QueryPartiPartiPLLDebug(ID1, ID2);
        }
    }
...
int Graph::QueryOverlayPLLDebug(int ID1, int ID2){
    if(!PartiTag[ID1].second || !PartiTag[ID2].second){
        cout<<"Not overlay vertex. "<<ID1<<"("<<PartiTag[ID1].second<<") "<<ID2<<"("<<PartiTag[ID2].second<<")"<<endl; exit(1);
    }
    ...
    bool ifFind=false;
    int lid,hid;
    int dis1Final, dis2Final, hubFinal;
    if(NodeOrder[ID1]>NodeOrder[ID2]){
        hid=ID1, lid=ID2;
    }else{
        hid=ID2, lid=ID1;
    ...
    cout<<ID1<<" "<<hub<<"("<<NodeOrder[hub]<<","<<PartiTag[hub].first<<") "<<ID2<<" : "<<dis1Final<<" "<<dis2Final<<" ; "<<dDijk_s<<" "<<dDijk_t<<endl;
    DijkstraPath(ID1,ID2,Neighbor);
    DijkstraPath(ID1,ID2,NeighborsOverlayV);
    if(!ifFind){
        cout<<"No common hub! "<<ID1<<" "<<ID2<<endl; exit(1);
    }
    return dis;
}
int Graph::QuerySamePartiPLL(int ID1, int ID2, vector<map<vertex,int>>& Labels){
...
//Case 3: Different partitions
int Graph::QueryPartiPartiPLLDebug(int ID1, int ID2){//both are within the partition
    int d=INF;
...
//        cout<<"Parti-Parti: "<<pid1<<" "<<pid2<<endl;
        ...
        int d_12=QueryOverlayPLL(b1,b2), dDijk_s=Dijkstra(ID1,b1,Neighbor), dDijk_12=Dijkstra(b1,b2,Neighbor), dDijk_t=Dijkstra(b2,ID2,Neighbor);
...
//            DijkstraPath(b2,ID2);
//        }
        int d_Dijk= Dijkstra(ID1,ID2,Neighbor);
        if(d!=d_Dijk){
            DijkstraPath(ID1,ID2,Neighbor);
        }
    }
...
void Graph::PPLLBatchUpdateDec(vector<pair<pair<int, int>, pair<int, int>>> &wBatch) {
    Timer tt;
    Timer tt2;
    tt.start();
...
//        cout<<"OverlayBatch size: "<<overlayBatch.size()<<endl;
    ...
    if(PSPStrategy>=PostBoundary){
        tt2.start();
        Repair_PartiIndexPLL(true, partiBatch);//post
        tt2.stop();
...
//function for throughput test of decrease update
void Graph::PPLLBatchUpdateDecPre(vector<pair<pair<int, int>, pair<int, int>>> &wBatch) {
    Timer tt;
    Timer tt2;
    tt.start();
    map<int, vector<pair<pair<int, int>, pair<int, int>>>> partiBatch; partiBatch.clear();
...
//function for throughput test of edge insertion update
void Graph::PPLLBatchUpdateEdgeInsert(vector<pair<pair<int, int>, pair<int, int>>> &wBatch, int batch_i, double &runT1) {
    Timer tt;
    Timer tt2;
    tt.start();
    map<int, vector<pair<pair<int, int>, pair<int, int>>>> partiBatch; partiBatch.clear();
...
//        cout<<"Update Partition number: "<<partiBatch.size()<<endl;
        ...
        for(auto it=partiBatch.begin();it!=partiBatch.end();++it){
            int pid=it->first;
            thread.add_thread(new boost::thread(&Graph::EdgeInsertPartiBatchUpdatePLL, this, pid, boost::ref(it->second), boost::ref(overlayBatch), true, boost::ref(updatedSCs[pid])));
//            cout<<"partition "<<pid<<endl;
//            EdgeInsertPartiBatchUpdatePLL(pid, it->second, overlayBatch, true, updatedSCs[pid]);
        }
        thread.join_all();
    }
...
//            cout<<a<<" "<<b<<" "<<oldW<<" "<<newW<<endl;
            bool ifNewEdge=false;
            bool ifUpdate=false;
            if(NeighborsOverlay[a].find(b)!=NeighborsOverlay[a].end()){//if found
                if(NeighborsOverlay[a][b]>newW){
                    NeighborsOverlay[a][b]=newW;
                    ifUpdate=true;
                }

            }else{
                cout<<"Not found overlay edge "<<a<<" "<<b<<endl;
                ifNewEdge=true;
            ...
            if(NeighborsOverlay[b].find(a)!=NeighborsOverlay[b].end()){//if found
                if(NeighborsOverlay[b][a]>newW){
                    NeighborsOverlay[b][a]=newW;
                    ifUpdate=true;
                }

            }else{
                cout<<"Not found overlay edge "<<b<<" "<<a<<endl;
                ifNewEdge=true;
                if(updateType!=EdgeInsertion && updateType!=VertexInsertion){
                    exit(1);
                }
            ...
            if(ifNewEdge){
                uNum++;
                EdgeInsertionPLL(a,b,newW,NeighborsOverlayV,Label);
            }
            else{
                if(ifUpdate){
                    uNum++;
                    ...
                    cout<<"Not trigger overlay update. "<<a<<" "<<b<<" "<<oldW<<"("<<NeighborsOverlay[a][b]<<") "<<newW<<endl;
                }
            }
...
//function for throughput test of edge insertion update
void Graph::PPLLBatchUpdateEdgeInsertPre(vector<pair<pair<int, int>, pair<int, int>>> &wBatch, int batch_i, double &runT1) {
    Timer tt;
    Timer tt2;
    tt.start();
    map<int, vector<pair<pair<int, int>, pair<int, int>>>> partiBatch; partiBatch.clear();
...
//        cout<<"Update Partition number: "<<partiBatch.size()<<endl;
        ...
        for(auto it=partiBatch.begin();it!=partiBatch.end();++it){
            int pid=it->first;
            thread.add_thread(new boost::thread(&Graph::EdgeInsertPartiBatchUpdatePLL, this, pid, boost::ref(it->second), boost::ref(overlayBatch), false, boost::ref(updatedSCs[pid])));
//            cout<<"partition "<<pid<<endl;
//            EdgeInsertPartiBatchUpdatePLL(pid, it->second, overlayBatch, true, updatedSCs[pid]);
        }
        thread.join_all();
...
//function for throughput test of vertex insertion update
void Graph::PPLLBatchUpdateVertexInsertPre(vector<pair<pair<int, int>, pair<int, int>>> &wBatch, int batch_i, double &runT1) {
    Timer tt;
    Timer tt2;
    tt.start();
    map<int, vector<pair<pair<int, int>, pair<int, int>>>> partiBatch; partiBatch.clear();
    vector<pair<pair<int, int>, pair<int, int>>> overlayBatch;
    map<pair<int,int>,pair<int,int>> updateSCTrue;
//    Neighbor.resize(node_num+wBatch.size());
    vector<int> newIDs;
    for(int k=0;k<wBatch.size();k++) {
        int a = node_num;
        int b = wBatch[k].first.first;
        int oldW = wBatch[k].second.first;
        int newW = wBatch[k].second.second;
//        cout<<"new edge 1. "<<a<<" "<<b<<" "<<newW<<endl;
        newIDs.push_back(a);
        vector<pair<vertex,int>> tempV;
        tempV.emplace_back(b,newW);
        Neighbor.emplace_back(tempV);
        NeighborsParti.emplace_back(tempV);
        PartiTag.emplace_back(PartiTag[b].first,0);//assert to the partition that b belongs
        ...
        Labels.push_back(unordered_map<int,int>());
//        Labels.push_back(map<int,int>());
        Neighbor[b].emplace_back(a,newW);
...
//        cout<<"Update Partition number: "<<partiBatch.size()<<endl;
        ...
        for(auto it=partiBatch.begin();it!=partiBatch.end();++it){
            int pid=it->first;
            thread.add_thread(new boost::thread(&Graph::VertexInsertPartiBatchUpdatePLL, this, pid, boost::ref(it->second), boost::ref(NeighborsParti)));
//            cout<<"partition "<<pid<<endl;
//            EdgeInsertPartiBatchUpdatePLL(pid, it->second, overlayBatch, true, updatedSCs[pid]);
        }
        thread.join_all();
...
//    CorrectnessCheck(100);
    int tv1=9531, tv2=204;
//    cout<<"Before step 2. "<<tv1<<" "<<tv2<<" "<<QueryPPLL(tv1,tv2)<<"("<<Dijkstra(tv1,tv2,NeighborsOverlayV)<<","<<Dijkstra(tv2,tv1,Neighbor)<<")"<<endl;
...
//        cout<<"new edge 2. "<<a<<" "<<b<<" "<<newW<<endl;
        ...
        int pid1=PartiTag[a].first, pid2=PartiTag[b].first;
        if(pid1 != pid2){
//            overlayBatch.emplace_back(make_pair(a,b), make_pair(oldW,newW));
            if(a<=b){
...
//            cout<<a<<" "<<b<<" "<<oldW<<" "<<newW<<endl;
            ...
            if(NeighborsOverlay[a].find(b)!=NeighborsOverlay[a].end()){//if found
                if(NeighborsOverlay[a][b]>newW){
                    int temp=Dijkstra(a,b,Neighbor);
                    if(newW < temp){
                        cout<<"Wrong update. "<<a<<" "<<b<<" "<<NeighborsOverlay[a][b]<<" "<<newW<<"("<<temp<<")"<<endl;
                    }
                    NeighborsOverlay[a][b]=newW;
                    ifUpdate=true;
                }
            ...
            if(NeighborsOverlay[b].find(a)!=NeighborsOverlay[b].end()){//if found
                if(NeighborsOverlay[b][a]>newW){
                    int temp=Dijkstra(b,a,Neighbor);
                    if(newW < temp){
                        cout<<"Wrong update. "<<b<<" "<<a<<" "<<NeighborsOverlay[b][a]<<" "<<newW<<"("<<temp<<")"<<endl;
                    }
                    NeighborsOverlay[b][a]=newW;
                    ifUpdate=true;
                }
...
//function for throughput test of vertex insertion update
void Graph::PPLLBatchUpdateVertexInsert(vector<pair<pair<int, int>, pair<int, int>>> &wBatch, int batch_i, double &runT1) {
    Timer tt;
    Timer tt2;
    tt.start();
    map<int, vector<pair<pair<int, int>, pair<int, int>>>> partiBatch; partiBatch.clear();
...
//        Labels.emplace_back(map<int,int>());
        PartiTag.emplace_back(PartiTag[b].first,0);//assert to the partition that b belongs
        for(int i=0;i<NodeOrder.size();++i){
            NodeOrder[i]=NodeOrder[i]+1;
        ...
        if(PSPStrategy>=PostBoundary){
            ...
            NeighborsPartiPostV.emplace_back(tempV);
            NeighborsPartiPost[b].insert({a,newW});
            NeighborsPartiPostV[b].emplace_back(a,newW);
            LabelsPost.emplace_back(unordered_map<int,int>());
        }
...
//partition update PLL
void Graph::EdgeInsertPartiBatchUpdatePLL(int pid, vector<pair<pair<int, int>, pair<int, int>>> &wBatch, vector<pair<pair<int, int>, pair<int, int>>> &weightOverlay, bool ifOverlay, vector<pair<pair<int,int>,int>>& updatedSC) {
...
//    vector<pair<pair<int,int>,int>> updatedSC;
    ...
    for(int i=0;i<wBatch.size();++i){
        ...
        EdgeInsertionPLL(ID1,ID2,newW,NeighborsParti,Labels);
    }
...
//                    sm->wait();
                    if(bid1<=bid2){
                        updatedSC.emplace_back(make_pair(bid1,bid2),newdis);//weightOverlay collect the changed edges on overlay graph
                    }else{
                        updatedSC.emplace_back(make_pair(bid2,bid1),newdis);//weightOverlay collect the changed edges on overlay graph
                    }
...
//function for maintaining the edge insertion updates of 2-hop labeling
void Graph::EdgeInsertionPLL(int a, int b, int newW, vector<vector<pair<vertex,int>>> &Neighbors,vector<unordered_map<vertex,int>> &Label){
...
//        cout<<"Do not trigger edge insertion update. "<<LID<<" "<<HID<<" "<<newW<<" "<<Dab<<endl;
    }
}

void Graph::EdgeInsertionPLL(int a, int b, int newW, vector<vector<pair<vertex,int>>> &Neighbors,vector<map<vertex,int>> &Label){
    //update the edges on original graph
    bool ifFind=false;
    for(int i=0;i<Neighbors[a].size();i++){
        if(Neighbors[a][i].first==b){
...
//function for maintaining the edge insertion updates of 2-hop labeling
void Graph::VertexInsertPartiBatchUpdatePLL(int pid, vector<pair<pair<int,int>,pair<int,int>>>& wBatch, vector<vector<pair<vertex,int>>> &Neighbors){
    int a,b,oldW,newW;
    for(int i=0;i<wBatch.size();++i){
        ...
        // label inherit
        int hub,dis;
        for(auto it=Labels[b].begin();it!=Labels[b].end();++it){
            hub=it->first, dis=it->second;
            Labels[a].insert({hub,dis+newW});
        }
        Labels[a].insert({a,0});
        if(PSPStrategy>=PostBoundary){
            for(auto it=LabelsPost[b].begin();it!=LabelsPost[b].end();++it){
                hub=it->first, dis=it->second;
                LabelsPost[a].insert({hub,dis+newW});
            }
            LabelsPost[a].insert({a,0});
        }
    }
}
...
//        cout<<"Not trigger overlay update. "<<LID<<" "<<HID<<" "<<oldW<<" "<<newW<<" "<<Dab<<endl;
    }
}

void Graph::DecreasePSL(int a, int b, int oldW, int newW, vector<vector<pair<vertex,int>>> &Neighbors,vector<map<vertex,int>> &Label){
    //update the edges on original graph
    for(int i=0;i<Neighbors[a].size();i++){
        if(Neighbors[a][i].first==b){
            Neighbors[a][i].second=newW;
...
int Graph::PLLQuery(int ID1,int ID2,vector<map<vertex,int>> &Label){
    int d=INF;
    int hub, dis1, dis2;
    for(auto it=Label[ID1].begin();it!=Label[ID1].end();it++){
        hub=(*it).first;
...
void Graph::EdgeDeletePartiBatchUpdatePLL(int pid, vector<pair<pair<int,int>,pair<int,int>>>& wBatch, vector<pair<pair<int,int>,pair<int,int>>>& weightOverlay, bool ifOverlay){
    //partition batch decrease update
//    vector<pair<pair<int,int>,int>> updatedSC;
    int ID1,ID2,oldW,newW;
    for(int i=0;i<wBatch.size();++i){
        ID1=wBatch[i].first.first; ID2=wBatch[i].first.second;
        oldW=wBatch[i].second.first; newW=wBatch[i].second.second;
        EdgeDeletePSL(ID1,ID2,oldW,NeighborsParti,Labels,PruningPointSetP,PruningPointSetP2);
    }
...
//    vector<pair<pair<int,int>,int>> updatedSC;
    ...
    for(int i=0;i<wBatch.size();++i){
        ...
        tt.stop();
//        cout<<ID1<<" "<<ID2<<" "<<oldW<<" "<<newW<<" : "<<tt.GetRuntime()<<" s."<<endl;
    }
...
void Graph::Repair_PartiIndexPLL(bool ifParallel, map<int, vector<pair<pair<int, int>, pair<int, int>>>> & partiBatch){
...
//            cout<<"Batch number: "<<processID[0].size()<<endl;
            ...
            if(updateType==EdgeIncrease){//increase update
                boost::thread_group thread;
                for(auto j=0;j<partiNum;++j){
            ...
            else if(updateType==EdgeDecrease || updateType==EdgeInsertion || updateType==VertexInsertion){//decrease update
                boost::thread_group thread;
                for(auto j=0;j<partiNum;++j){
            ...
            else if(updateType==EdgeDeletion || updateType==VertexDeletion){
                boost::thread_group thread;
                for(auto j=0;j<partiNum;++j){
                    thread.add_thread(new boost::thread(&Graph::RepairPartitionIndexEdgeDeletePLL, this, j, boost::ref(partiBatch)));
                }
                thread.join_all();
            }
        }
        ...
        // single thread
        if(updateType==EdgeIncrease){
            for(int k=0;k<partiNum;k++){
//                cout<<"Repairing partition "<<k<<endl;
                RepairPartitionIndexIncreasePLL(k,partiBatch);
            }
        }
        else if(updateType==EdgeDecrease || updateType==EdgeInsertion || updateType==VertexInsertion){
            for(int k=0;k<partiNum;k++){
//                cout<<"Repairing partition "<<k<<endl;
            ...
            for(int k=0;k<partiNum;k++){
//                cout<<"Repairing partition "<<k<<endl;
                RepairPartitionIndexEdgeDeletePLL(k,partiBatch);
            }
        }
...
//            DecreasePSL(a,b,oldW,newW, NeighborsPartiPostV, LabelsPost);
            ...
            bool ifUpdate=false;
            if(NeighborsPartiPost[a].find(b)!=NeighborsPartiPost[a].end()){//if found
                if(NeighborsPartiPost[a][b]>newW){
                    NeighborsPartiPost[a][b]=newW;
                    ifUpdate=true;
                }

            }else{
//                cout<<"Not found post edge "<<a<<" "<<b<<endl;
                ifNewEdge=true;
                if(updateType!=EdgeInsertion && updateType!=VertexInsertion){
                    exit(1);
                }
            ...
            if(NeighborsPartiPost[b].find(a)!=NeighborsPartiPost[b].end()){//if found
                if(NeighborsPartiPost[b][a]>newW){
                    NeighborsPartiPost[b][a]=newW;
                    ifUpdate=true;
                }

            }else{
//                cout<<"Not found post edge "<<b<<" "<<a<<endl;
                ifNewEdge=true;
                if(updateType!=EdgeInsertion && updateType!=VertexInsertion){
                    exit(1);
                }
            ...
            if(ifNewEdge){
                EdgeInsertionPLL(a,b,newW,NeighborsPartiPostV, LabelsPost);
            }
            else{
                if(ifUpdate){
                    DecreasePSL(a,b,oldW,newW, NeighborsPartiPostV, LabelsPost);
                }else{
                    cout<<"Not trigger post update. "<<a<<" "<<b<<" "<<oldW<<"("<<NeighborsOverlay[a][b]<<") "<<newW<<endl;
                }
            }
        }
...
void Graph::RepairPartitionIndexPLLV(vector<int>& p, bool ifIncrease, map<int, vector<pair<pair<int, int>, pair<int, int>>>> & partiBatch) {
    ...
    if(updateType==EdgeIncrease){
        ...
        for(int i=0;i<p.size();++i){
            RepairPartitionIndexEdgeDeletePLL(p[i], partiBatch);
        }
    }

}
//function for edge increase update of post-strategy
...
void Graph::RepairPartitionIndexEdgeDeletePLL(int pid, map<int, vector<pair<pair<int, int>, pair<int, int>>>> & partiBatch) {
    int ID1,ID2,woverlay;
    int wlocal=INF;
    //boundary edges
    vector<pair<pair<int,int>,pair<int,int>>> weightsParti;//collect the changed edges on overlay graph
...
//                NeighborsPartiPost[a].erase(b);
            ...
            if (NeighborsPartiPost[b].find(a) != NeighborsPartiPost[b].end()) {//if found
                NeighborsPartiPost[b][a] = newW;
//                NeighborsPartiPost[b].erase(a);
            } else {
                cout << "Wrong. Not found " << b << " " << a << endl;
                exit(1);
            }
//            EdgeDeletePSL(a,b,oldW, NeighborsPartiPostV, LabelsPost, PruningPointSetPost, PruningPointSetPost2);
            IncreasePSL(a,b,oldW,newW, NeighborsPartiPostV, LabelsPost, PruningPointSetPost, PruningPointSetPost2);
        }
        ifRepaired[pid]=true;
    }
...
//function for throughput test of edge deletion update
void Graph::PPLLBatchUpdateEdgeDelete(vector<pair<pair<int, int>, pair<int, int>>> &wBatch) {
    Timer tt;
    Timer tt2;
    tt.start();
    map<int, vector<pair<pair<int, int>, pair<int, int>>>> partiBatch; partiBatch.clear();
...
//                Neighbor[b][i].second=newW;
    ...
    if(!overlayBatch.empty()){
        int a,b,oldW,newW;
        for(int i=0;i<overlayBatch.size();++i) {
            a = overlayBatch[i].first.first;
            b = overlayBatch[i].first.second;
            oldW = overlayBatch[i].second.first;
            newW = overlayBatch[i].second.second;
            if(NeighborsOverlay[a].find(b)!=NeighborsOverlay[a].end()){//if found
                NeighborsOverlay[a][b]=newW;
            }else{
                cout<<"Wrong. Not found "<<a<<" "<<b<<endl; exit(1);
            ...
            EdgeDeletePSL(a, b, oldW, NeighborsOverlayV, Label, PruningPointSet, PruningPointSet2);
        }
        overlayBatch.clear();
    }
    vUpdated.assign(node_num, false);
...
//        cout<<"Update Partition number: "<<partiBatch.size()<<endl;
        ...
        for(auto it=partiBatch.begin();it!=partiBatch.end();++it){
            int pid=it->first;
            thread.add_thread(new boost::thread(&Graph::EdgeDeletePartiBatchUpdatePLL, this, pid, boost::ref(it->second), boost::ref(overlayBatch), true));
        }
        thread.join_all();
    }
    ...
    if(!overlayBatch.empty()){
        ...
        for(int i=0;i<overlayBatch.size();++i){
            ...
            if(NeighborsOverlay[a].find(b)!=NeighborsOverlay[a].end()){//if found
                NeighborsOverlay[a][b]=newW;
//                NeighborsOverlay[a].erase(b);
            }else{
                cout<<"Wrong. Not found "<<a<<" "<<b<<endl; exit(1);
            }
            if(NeighborsOverlay[b].find(a)!=NeighborsOverlay[b].end()){//if found
                NeighborsOverlay[b][a]=newW;
//                NeighborsOverlay[b].erase(a);
            }else{
                cout<<"Wrong. Not found "<<b<<" "<<a<<endl; exit(1);
            }
...
void Graph::PPLLBatchUpdateEdgeDeletePre(vector<pair<pair<int, int>, pair<int, int>>> &wBatch) {
    Timer tt;
    Timer tt2;
    tt.start();
    map<int, vector<pair<pair<int, int>, pair<int, int>>>> partiBatch; partiBatch.clear();
...
//        cout<<"Update Partition number: "<<partiBatch.size()<<endl;
        ...
        for(auto it=partiBatch.begin();it!=partiBatch.end();++it){
            int pid=it->first;
            thread.add_thread(new boost::thread(&Graph::EdgeDeletePartiBatchUpdatePLL, this, pid, boost::ref(it->second), boost::ref(overlayBatch), false));
        }
        thread.join_all();
    }
...
//function for throughput test of increase update
void Graph::PPLLBatchUpdateInc(vector<pair<pair<int, int>, pair<int, int>>> &wBatch) {
    Timer tt;
    Timer tt2;
    tt.start();
    map<int, vector<pair<pair<int, int>, pair<int, int>>>> partiBatch; partiBatch.clear();
...
//            cout<<"partition "<<pid<<endl;
//            IncreasePartiBatchUpdatePLL(pid, it->second, overlayBatch, true);
        }
        thread.join_all();
    }
    tt2.stop();
//    cout<<"Coarse update time: "<<coaseUpdateT<<" ; refine update time: "<<refineUpdateT<<" ; clean update time: "<<cleanUpdateT<<" ; DisQueryVally2 number: "<<queryNum<<" ; Label change number: "<<labelChangeNum<< endl;
//    cout<<"Time for partition index update: "<<tt2.GetRuntime()<<" s."<<endl;
    tt2.start();
    if(!overlayBatch.empty()){
        cout<<"OverlayBatch size: "<<overlayBatch.size()<<endl;
        int a,b,oldW,newW;
    ...
    tt2.stop();
//    cout<<"Coarse update time: "<<coaseUpdateT<<" ; refine update time: "<<refineUpdateT<<" ; clean update time: "<<cleanUpdateT<<" ; DisQueryVally2 number: "<<queryNum<<" ; Label change number: "<<labelChangeNum<<endl;
//    cout<<"Time for overlay index update: "<<tt2.GetRuntime()<<" s."<<endl;
...
//function for throughput test of increase update
void Graph::PPLLBatchUpdateIncPre(vector<pair<pair<int, int>, pair<int, int>>> &wBatch) {
    Timer tt;
    Timer tt2;
    tt.start();
    map<int, vector<pair<pair<int, int>, pair<int, int>>>> partiBatch; partiBatch.clear();
...
void Graph::EdgeDeletePSL(int a, int b, int oldW, vector<vector<pair<vertex,int>>> &Neighbor,vector<unordered_map<vertex,int>> &Label, vector<unordered_map<vertex,unordered_set<vertex>>> &PruningPointNew, vector<unordered_map<vertex,vertex>>& PruningPointSet2){//
    for(int i=0;i<Neighbor[a].size();i++){
        if(Neighbor[a][i].first==b){
...
//        cout<<"Remove and add pruning point."<<endl;
        ...
        if(ifDebug){
            cout<<"Not triggered! "<<LID<<" "<<HID<<": "<<dislower<<" "<<dispeak<<" "<<oldW<<endl;
        }
    }

}

void Graph::EdgeDeletePSL(int a, int b, int oldW, vector<vector<pair<vertex,int>>> &Neighbor,vector<map<vertex,int>> &Label, vector<unordered_map<vertex,unordered_set<vertex>>> &PruningPointNew, vector<unordered_map<vertex,vertex>>& PruningPointSet2){//
    for(int i=0;i<Neighbor[a].size();i++){
        if(Neighbor[a][i].first==b){
//            cout<<"Core "<<a<<" "<<b<<" "<<Neighbor[a][i].second<<" "<<newW<<endl;
//            Neighbor[a][i].second=newW;
...
//            cout<<"After AL1: "<<AL1.size()<<" "<<AL2Check.size()<<" "<<WaitPro.size()<<" "<<WaitProP.size()<< endl;
            tt.stop();
//            coaseUpdateT+=tt.GetRuntime();
//            cout<<"Coarse update time: "<<tt.GetRuntime()<<" "<<LID<<" "<<HID<<" "<<oldW<<endl;
//            if(ifDebug){
//                vallyPair= DisQueryVally2(lid,hid,Neighbor,Label);
//                disvally=vallyPair.first, vallyID=vallyPair.second;
//                peakPair= DisQueryPeak2(lid,hid,Label);
...
//            }
            ...
            RefineUpdate( WaitPro, WaitProP, AL1, AL2, AL2Check, outdatedPruning, newPruningPoints, PruningPointSet2, Neighbor, Label, PruningPointNew, ifDebug, lid, hid, ChangedLabels, NoSupportedPair);
            tt.stop();
//            refineUpdateT+=tt.GetRuntime();
//            cout<<"Refined update time: "<<tt.GetRuntime()<<endl;
//            cout<<"After AL2: "<<AL1.size()<<" "<<AL2Check.size()<<" "<<WaitPro.size()<<" "<<WaitProP.size()<<endl;
        }
//        cout<<"Done."<<endl;
...
//        cout<<"Remove and add pruning point."<<endl;
        ...
        PPRClean(Neighbor, newPruningPoints, outdatedPruning, ifDebug, lid, hid, Label, PruningPointNew, PruningPointSet2, ChangedLabels);
        tt.stop();
//        cleanUpdateT+=tt.GetRuntime();
//        cout<<"Clean time: "<<tt.GetRuntime()<<endl;
    }
    else{
        if(ifDebug){
            cout<<"Not triggered! "<<LID<<" "<<HID<<": "<<dislower<<" "<<dispeak<<" "<<oldW<<" "<<newW<<endl;
...
void Graph::IncreasePSL(int a, int b, int oldW, int newW, vector<vector<pair<vertex,int>>> &Neighbor,vector<map<vertex,int>> &Label, vector<unordered_map<vertex,unordered_set<vertex>>> &PruningPointNew, vector<unordered_map<vertex,vertex>>& PruningPointSet2){//
    for(int i=0;i<Neighbor[a].size();i++){
        if(Neighbor[a][i].first==b){
//            cout<<"Core "<<a<<" "<<b<<" "<<Neighbor[a][i].second<<" "<<newW<<endl;
            Neighbor[a][i].second=newW;
...
//        AL1.emplace_back(LID,HID);
        WaitPro.push(make_pair(LID, make_pair(HID,Label[LID][HID])));
        if(disvally>=INF){
            Label[LID].erase(HID);//correct to the new value
        }else{
            Label[LID][HID]=disvally;//correct to the new value
        }
...
//            vallyPair= DisQueryVallyVert2(HID,hubID,Neighbor,Label);
            disvally=vallyPair.first, vallyID= vallyPair.second;
...
//                }
                ...
                if(disvally>=INF){
                    Label[HID].erase(hubID);//correct to the new value
                }else{
...
//            vallyPair= DisQueryVallyVert2(LID,hubID,Neighbor,Label);
            disvally=vallyPair.first, vallyID= vallyPair.second;
...
//                }
                ...
                if(disvally>=INF){
                    Label[LID].erase(hubID);//correct to the new value
                }else {
...
//    cout<<"Before while loop. DisQueryVally2 number: "<<queryNum<<endl;
    unsigned long long int waitProNum=0;
    unsigned long long int labelCNum=0;
    while(!WaitPro.empty()){
        queue<pair<int,pair<int,int>>> WaitProTem;
        waitProNum+=WaitPro.size();
        while(!WaitPro.empty()){
...
//                    vallyPair = DisQueryVallyVert2(neiID, hubID, Neighbor, Label);
                    disvally = vallyPair.first, vallyID = vallyPair.second;
...
//                    }
                    ...
                    if (Label[neiID][hubID] < disvally) {//original setting
                        ...
                        if(ChangedLabels[neiID].find(hubID)!=ChangedLabels[neiID].end()){
//                            if(neiID==5048 && hubID==5139){
////                            if(neiID==5111 && hubID==5112){
//                                cout<<neiID<<"("<<NodeOrder[neiID]<<") "<<hubID<<"("<<NodeOrder[hubID]<<") "<<Label[neiID][hubID]<<" "<<disvally<<" "<<vallyID<<"("<<NodeOrder[vallyID]<<")"<< endl;
//                            }
                        ...
                        if(disvally>=INF){
                            Label[neiID].erase(hubID);//correct to the new value
                        }else {
...
//    cout<<"WaitPro number: "<<waitProNum<<" ; change label number: "<<labelCNum<<endl;
}
...
//                    vallyPair = DisQueryVallyVert2(neiid, u, Neighbor, Label);
                    disvally = vallyPair.first; vallyID = vallyPair.second;
...
//                vallyPair=DisQueryVallyVert2(s,curID,Neighbor,Label);
...
//                vallyPair=DisQueryVallyVert2(curID,s,Neighbor,Label);
                disvally=vallyPair.first; vallyID=vallyPair.second;
...
//                    cout<<"New Pruning point erase wrong. "<<id1<<" "<<id2<<" "<<Label[id1][id2]<<" "<<disvally<<" "<<dispeak<<"("<<peakhub<<","<<hub<<") "<<DijkstraCore(id1,id2)<<endl;
    ...
    for(int i=node_num-1;i>=0;--i){
        ...
        if(!ChangedLabels[id1].empty()){
            labelChangeNum+=ChangedLabels[id1].size();
...
//                        Label[id1][id2]=disvally;
                        ...
                        cout<<"incorrect vally label! L("<<id1<<","<<id2<<"): "<<Label[id1][id2]<<" "<<disvally<<" "<<Dijkstra(id1,id2,this->Neighbor)<<endl;
//                        Label[id1][id2]=disvally;
                    }
                }
            }
...
//function for cleaning redundant labels and PPR
void Graph::PPRClean(vector<vector<pair<vertex,int>>> &Neighbor, map<pair<vertex,vertex>,vertex> &newPruningPoints, set<tuple<vertex,vertex,vertex>> &outdatedPruning, bool ifDebug, int lid, int hid, vector<map<vertex,int>> &Label, vector<unordered_map<vertex,unordered_set<vertex>>>& PruningPointSet, vector<unordered_map<vertex,vertex>>& PruningPointSet2, vector<unordered_set<int>>& ChangedLabels){
    int dislower,disvally,dispeak;
    vertex peakhub,vallyID;
    pair<int,vertex> peakPair;//distance, hubID
    pair<int,vertex> vallyPair;//distance, vallyID
...
int Graph::DisQueryLower1(int ID1, int ID2, vector<vector<pair<vertex,int>>> &Neighbors,vector<map<vertex,int>> &Label){
    int neiID,neiDis;
    int d=INF;
    for(int i=0;i<Neighbors[ID1].size();i++){
        neiID=Neighbors[ID1][i].first;
...
int Graph::DisQueryVally(int ID1, int ID2, vector<vector<pair<vertex,int>>> &Neighbors,vector<map<vertex,int>> &Label){
    int neiID,neiDis;
    int d=INF;
    for(int i=0;i<Neighbors[ID1].size();i++){
        neiID=Neighbors[ID1][i].first;
    ...
    for(int i=0;i<Neighbors[ID1].size();i++){
        ...
        if(NodeOrder[neiID]<=NodeOrder[ID2] && Label[neiID].find(ID2)!=Label[neiID].end()){
            ...
            if(neiDis+Label[neiID][ID2]<d){
                ...
                finalNeigh = neiID;
//                cout<<neiID<<" "<<d<<endl;
//                if(ID1==5048 && ID2==5139){
////                if(ID1==5111 && ID2==5112){
//                    cout<<"Find. "<<neiID<<"("<<NodeOrder[neiID]<<") "<<neiDis<<" "<<d<<endl;
//                }
            }
        }
    }
...
//function of updating the label with the changed edge, 2024-08-11
    ...
    for(int i=0;i<Neighbors[ID1].size();i++){
        ...
        if(NodeOrder[neiID]<=NodeOrder[ID2] && Label[neiID].find(ID2)!=Label[neiID].end()){//if found
            if(neiDis+Label[ID1][ID2] != Label[neiID][ID2]){//avoid the case that the shortest path between neiID and ID2 pass L(ID1,ID2)
                ...
                if(dnei<=Label[neiID][ID2]){//exist another supportive neighbor
                    if(neiDis+Label[neiID][ID2]<d){
                        d=neiDis+Label[neiID][ID2];
                        finalNeigh = neiID;
                    }
...
pair<int,int> Graph::DisQueryVally2(int ID1, int ID2, vector<vector<pair<vertex,int>>> &Neighbors,vector<map<vertex,int>> &Label){
    int neiID,neiDis;
    int d=INF;
    int finalNeigh = -1;
    for(int i=0;i<Neighbors[ID1].size();i++){
...
int Graph::DisQueryPeak(int ID1, int ID2,vector<map<vertex,int>> &Label){
    int d=INF;
...
pair<int,int> Graph::DisQueryPeak2(int ID1, int ID2,vector<map<vertex,int>> &Label){
    int d=INF;
