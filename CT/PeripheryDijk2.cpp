...
void Graph::PartitionPostProcess(){
	...
	vecp.clear();
	unordered_map<int,vector<pair<int,int>>> vecvecpair;
    vecvecpair.clear();
//	vecvecpair.assign(nodenum,vecp);
	AdjaPartiM.assign(partiNum,vecvecpair);
	...
	for(int NodeID=0;NodeID<nodenum;NodeID++){
		if(CoreTag[NodeID]==-1){//core vertex + boundary vertex
			if(BoundTag[NodeID]==1){//boundary vertex
				...
				for(int nei=0;nei<Neighbor[NodeID].size();nei++){
					...
					if(CoreTag[neiID]==-1){//**********neighbor is core vertex or boundary vertex
                        ...
                        if(AdjaPartiM[pid].find(NodeID) == AdjaPartiM[pid].end()){//if not found
                            AdjaPartiM[pid].insert({NodeID,vector<pair<int,int>>()});
                        }
						AdjaPartiM[pid][NodeID].push_back(Neighbor[NodeID][nei]);
			...
			for(int k=0;k<Neighbor[NodeID].size();k++)
				AdjaPartiM[pid][NodeID].push_back(Neighbor[NodeID][k]);

			//PartiVertexInverted[pid].insert(make_pair(NodeID,AdjaPartiM[pid].size()));
			//AdjaPartiM[pid].push_back(Neighbor[NodeID]);
		}
	}
	...
	for(int pid=0;pid<partiNum;pid++){
		...
		for(int i=0;i<BoundVertex[pid].size();i++){
            ...
            if(AdjaPartiM[pid].find(ID1) == AdjaPartiM[pid].end()){//if not found
                continue;
            }
			if(AdjaPartiM[pid][ID1].size()!=0){
			//if(PartiVertexInverted[pid].find(ID1)!=PartiVertexInverted[pid].end()){//only if ID1 has neighbors in partition pid
				for(int j=i+1;j<BoundVertex[pid].size();j++){
                    ...
                    if(AdjaPartiM[pid].find(ID2) == AdjaPartiM[pid].end()){//if not found
                        continue;
                    }
					if(AdjaPartiM[pid][ID2].size()!=0){
...
int Graph::DijkstraParti(int ID1, int ID2, int pid){
	...
	while(!pqueue.empty()){
        ...
        if(AdjaPartiM[pid].find(topNodeID) != AdjaPartiM[pid].end()){//if found
            for(it=AdjaPartiM[pid][topNodeID].begin();it!=AdjaPartiM[pid][topNodeID].end();it++){
                NNodeID=(*it).first;
            ...
            cout<<"AdjaPartiM wrong! "<<topNodeID<<" does not exist in partition "<<pid<<endl;
        }
...
//		DecreasePLL(a,b,oldW,newW,AdjaCore,IndexCore);
		...
		if(CoreTag[a]!=-1)
			...
			pid=CoreTag[b];
        if(AdjaPartiM[pid].find(a) != AdjaPartiM[pid].end()){//if found
            //change the partition graph
            for(int k=0;k<AdjaPartiM[pid][a].size();k++){
                if(AdjaPartiM[pid][a][k].first==b){
                    AdjaPartiM[pid][a][k].second=newW;
                    break;
                }
            }
        }

        if(AdjaPartiM[pid].find(b) != AdjaPartiM[pid].end()){//if found
            for(int k=0;k<AdjaPartiM[pid][b].size();k++){
                if(AdjaPartiM[pid][b][k].first==a){
                    AdjaPartiM[pid][b][k].second=newW;
                    break;
                }
            }
        }
...
//		IncreasePLL(a,b,oldW,newW,AdjaCore,IndexCore,PruningPointCore,NoSupportedPairCore);
        ...
        if(AdjaPartiM[pid].find(b) != AdjaPartiM[pid].end()){
            for(int k=0;k<AdjaPartiM[pid][b].size();k++){
                if(AdjaPartiM[pid][b][k].first==a){
                    AdjaPartiM[pid][b][k].second=newW;
                    break;
