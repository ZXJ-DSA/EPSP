...
void Graph::PartitionPostProcess(){
	...
	vecvecpair.clear();
    cout<<"Flag 1."<<endl;
	AdjaPartiM.assign(partiNum,vecvecpair);//hug memory consumption!!
    cout<<"Flag 2."<<endl;
...
void Graph::Decrease(int a, int b, int oldW, int newW){
	...
	if((CoreTag[a]==-1 && BoundTag[a]==0) || (CoreTag[b]==-1 && BoundTag[b]==0)){//edges in the core
		...
		DecreaseH2H(a,b,newW,AdjaPartiM[pid],Trees[pid],ranks[pid],heightMaxs[pid]);
		//cout<<">>>>>>>>>>"<<endl;
...
void Graph::Increase(int a, int b, int oldW, int newW){
	...
	if((CoreTag[a]==-1 && BoundTag[a]==0) || (CoreTag[b]==-1 && BoundTag[b]==0)){//edges in the core
		...
		//cout<<"<<<<<<<<<<"<<endl;
		//cout<<CoreTag[a]<<" "<<BoundTag[a]<<" "<<CoreTag[b]<<" "<<BoundTag[b]<<" pid "<<pid<<endl;
		IncreaseH2H(a,b,oldW,newW,AdjaPartiM[pid],Trees[pid],ranks[pid],heightMaxs[pid],SCconNodesMTs[pid],VidtoTNids[pid]);
		//cout<<">>>>>>>>>>"<<endl;
