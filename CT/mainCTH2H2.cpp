...
//    cout<<"partition ID ";
    ...
    if(g.ifParallel){
        g.Trees.resize(g.partiNum);
        g.ranks.resize(g.partiNum);
        g.heightMaxs.resize(g.partiNum);
        g.SCconNodesMTs.resize(g.partiNum);
        g.VidtoTNids.resize(g.partiNum);
        t10=std::chrono::high_resolution_clock::now();
        g.H2HindexParallel(true);
...
//        h.Neighbors=g.AdjaParti[partiID];
            ...
            for(auto it=g.AdjaPartiM[partiID].begin();it!=g.AdjaPartiM[partiID].end();++it){
                ID = it->first;
                if(it->second.empty()){
...
//    cout<<"Core's Index Construction Finished"<<endl;
    ...
    cout<<"Overall Construction Time: "<<runT1+runT2+runT3+runT4+maxrT<<" s."<<endl;
    cout<<"Minimum Overall Construction Time: "<<runT1+runT3+runT4+maxrT<<" s."<<endl;
