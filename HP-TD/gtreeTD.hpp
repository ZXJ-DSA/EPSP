...
//		printf("COMPLETE.\n");
        ...
        for ( set<int>::iterator it = current.nset.begin(); it != current.nset.end(); it++ ){
            ...
            childset[slot].insert(*it);//get each child partition
        }
...
vector<int> Gtree::dijkstra_candidate_No( int s, vector<int> &cands, unordered_map<int,vector<pair<int,int>>> &graph ){
    // init
    set<int> todo;
    todo.clear();
    todo.insert(cands.begin(), cands.end());//get the partition vertex set
...
void Gtree::hierarchy_shortest_path_calculation(bool ifParallel){
    ...
    cout<<"Leaf node number: "<<leafNodes.size()<<endl;
    Timer tt;
    tt.start();
        ...
        if(partiNum>thread_num){
            ...
            for(int i=0;i<thread_num;i++){
                ...
                if(i==thread_num-1)
                    ...
                    p.second=(i+1)*step;
                threadf.add_thread(new boost::thread(&Gtree::leafNodeAllPair, this, p, boost::ref(leafNodes),boost::ref(graph)));
            }
            threadf.join_all();
        }else{
            boost::thread_group threadf;
            for(int pid=0;pid<partiNum;++pid) {
                threadf.add_thread(new boost::thread(&Gtree::leafNodeAllPair, this, make_pair(pid,pid+1), boost::ref(leafNodes),boost::ref(graph)));
            }
            threadf.join_all();
        }
...
//                    graph[s].adjweight.push_back( vertex_pairs[s][t] );
                    ...
                    if(E[s].find(t) != E[s].end()){//if found
                        if(E[s][t].first == vertex_pairs[s][t]){//if found
                            E[s][t].second += 1;
                        }else{//if not found
...
void Gtree::hierarchy_shortest_path_calculation_NoBoundary(bool ifParallel){
    //get partition graph
    getPartiGraph();
        ...
        if(partiNum>thread_num){
            ...
            for(int i=0;i<thread_num;i++){
                ...
                if(i==thread_num-1)
                    ...
                    p.second=(i+1)*step;
                threadf.add_thread(new boost::thread(&Gtree::leafNodeAllPair_No, this, p, boost::ref(leafNodes),boost::ref(PartiGraph)));
            }
            threadf.join_all();
        }else{
            boost::thread_group threadf;
            for(int pid=0;pid<partiNum;++pid) {
                threadf.add_thread(new boost::thread(&Gtree::leafNodeAllPair_No, this, make_pair(pid,pid+1), boost::ref(leafNodes),boost::ref(PartiGraph)));
            }
            threadf.join_all();
        }
        if(!vSmNode.empty()){
...
//function of computing the distances between boundary vertex to all other vertex in the leaf node
void Gtree::leafNodeAllPair(pair<int,int> p, vector<int> & leafNodes,vector<Node>& graph){
    vector<int> cands;
    vector<int> result;
    unordered_map<int, unordered_map<int,int> > vertex_pairs;//result of distance matrix
    int s, t, tn, nid, cid, weight;
...
//                    graph[s].adjweight.push_back( vertex_pairs[s][t] );
                /// For NeighborCon
                if(E[s].find(t) != E[s].end()){//if found
                    if(E[s][t].first == vertex_pairs[s][t]){
                        E[s][t].second += 1;
                    }else{
                        assert(E[s][t].first > vertex_pairs[s][t]);
                        E[s][t].first = vertex_pairs[s][t];
...
void Gtree::leafNodeAllPair_No(pair<int,int> p, vector<int> & leafNodes, map<int,unordered_map<int,vector<pair<int,int>>>> & graphs){
    vector<int> cands;
    vector<int> result;
    unordered_map<int, unordered_map<int,int> > vertex_pairs;//result of distance matrix
    int s, t, tn, nid, cid, weight;
...
//    hierarchy_shortest_path_calculation(false);
//    TIME_TICK_END
//    TIME_TICK_PRINT("MIND")
//    cout << "Done."<<endl;
    tt.stop();
    t2 = tt.GetRuntime();
    cout << "The time for index computing: " << t2 << " s." << endl;
...
void Gtree::ReadUpdates(string filename){
    ...
    for(int i=0;i<num;i++){
        inFile>>ID1>>ID2>>weight;
        updateEdges.emplace_back(make_pair(ID1, ID2), make_pair(weight,0));
    }
    inFile.close();
}
...
//            }
            cout<<" " <<ID1 << " "<<ID2;
            oldW = updateEdges[i].second.first;
            switch (updateType) {
                case INCREASE:
                ...
                case DECREASE:
                    newW = (int)(0.5*oldW);
                    if(newW <=0){
                        cout<<"Wrong new edge weight: "<<oldW<<" "<<newW<<endl;
                        exit(1);
                    }
                    break;
                case MIX:
            ...
            cout<<" ("<<oldW<<"->"<<newW<<") "<<endl;
            updateEdges[i].second.second = newW;
            vector<pair<pair<int,int>,pair<int,int> > > testData;
            testData.emplace_back(updateEdges[i]);
            ///update
            tt.start();
            if(Nodes[ID1].inleaf == Nodes[ID2].inleaf){//if in the same partition
//                cout<<"Same-parti"<<endl;
                //update this partition first
                int PID = Nodes[ID1].inleaf;
                map<pair<int,int>,pair<int,int>> affectedBPairs;
//                PartitionUpdate_No(PID,affectedBPairs,ifParallel);
                ...
                if(!affectedBPairs.empty()){
                    ...
                    for(auto it=affectedBPairs.begin();it!=affectedBPairs.end();++it){
                        testData2.emplace_back(it->first,it->second);
                    }
                    if(updateType == DECREASE){
                        H2HdecBat(testData2);
                    }else if(updateType == INCREASE){
                        H2HincBatMT(testData2);
                    }
                }
            }
            else{//if in different partition, must be the boundary vertex
//                cout<<"Parti-Parti "<<Nodes[ID1].inleaf<<" "<<Nodes[ID2].inleaf<<endl;
                if(!BoundaryTag[ID1] || !BoundaryTag[ID2]){
                    cout<<"Wrong! "<<ID1<<" "<<ID2<<" "<<BoundaryTag[ID1]<<" "<<BoundaryTag[ID2]<<endl;
                    exit(1);
                }
                if(updateType == DECREASE){
                    H2HdecBat(testData);
...
void Gtree::PartitionUpdate_No(int tn, map<pair<int,int>,pair<int,int>> & affectedBPairs, bool ifParallel){
        ...
        if(bNum>thread_num){
            ...
            for(int i=0;i<thread_num;i++){
                ...
                if(i==thread_num-1)
                    ...
                    p.second=(i+1)*step;
                threadf.add_thread(new boost::thread(&Gtree::boundaryVUpdate, this, p, tn, boost::ref(cands),boost::ref(affectedBPairs)));
            }
            threadf.join_all();
        }else{
            boost::thread_group threadf;
            for(int pid=0;pid<bNum;++pid) {
                threadf.add_thread(new boost::thread(&Gtree::boundaryVUpdate, this, make_pair(pid,pid+1), tn, boost::ref(cands),boost::ref(affectedBPairs)));
            }
            threadf.join_all();
        }
    }
...
//                result = dijkstra_candidate( GTree[tn].union_borders[k], cands, graph );//distance vector from s to all borders
            ...
            for ( int p = 0; p < result.size(); ++p ){
                if(result[p] != GTree[tn].mind[k*vNum + p]){
                    if(BoundaryTag[cands[p]]){//if it is boundary vertex
                        if(ID<cands[p]){
...
void Gtree::boundaryVUpdate(pair<int,int> p, int tn, vector<int> & cands, map<pair<int,int>,pair<int,int>> & affectedBPairs){
    int vNum = cands.size();
    int ID;
...
void Gtree::SaveIndex(){
    string fileTree = DataPath + "/" +dataset+"/tmp/" +dataset+".gtree";
    string fileTD = DataPath + "/" +dataset+"/tmp/" +dataset+".td";
    string fileOrder = DataPath + "/" +dataset+"/tmp/" +dataset+".gtree";
    for(int i=0;i<Tree.size();++i){

    }
}

void Gtree::IndexSize() {
    long long m=0;

    for(int i=0;i<GTree.size();++i){
        m+=GTree[i].mind.size()*sizeof(int);
    }
...
//        s=759; t=760;
        ...
        if(dis_Dijk!=dis_GTree){
            cout<<"InCorrect!! "<<i<<" "<<s<<" "<<t<<" "<<dis_GTree<<" "<<dis_Dijk<<endl;
            QueryDebug(s,t);
        }
    ...
    cout << "Average run time of HP-TD: "<<time_PLL*1000/times<<" ms."<<endl;
