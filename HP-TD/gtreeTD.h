...
public:
    ...
    vector<Node> Nodes; // vector of vertex, contain adjacency list
...
//    vector<vector<pair<int,int>>> Neighbor;//adjacency list
    vector<int> NodeOrder;
    vector<int> vNodeOrder;
    // contraction
    vector<Semaphore*> vSm;
    Semaphore* sm = new Semaphore(thread_num);
...
//    void getBOrder(int curID,vector<int> & HighToLow);
    void MDEContract();
    void LayeredMDEContract();
    void Contraction();
    void leafNodeAllPair(pair<int,int> p, vector<int> & leafNodes,vector<Node>& graph);
...
//    vector<map<int,pair<int,int>>> E;
    vector<unordered_map<int,pair<int,int>>> E;
