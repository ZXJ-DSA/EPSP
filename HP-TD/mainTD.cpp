...
//    ifParallel = false;
    ...
    if( argc != 5 && argc != 6){
        ...
        printf("<arg4> task type, e.g. 1: G-Tree building; 2: Query processing; 3: G-Tree update; 5: generate query OD pairs; 6: generate update OD pairs. default: 1\n");
    ...
    if(argc > 1) {
        cout << "argc: " << argc << endl;
        cout << "argv[1]: " << argv[1] << endl;//data path
        DataPath = argv[1];
        if(argc > 2){
        ...
        if(argc > 3){
            cout << "argv[3]: " << argv[3] << endl;//partition number
