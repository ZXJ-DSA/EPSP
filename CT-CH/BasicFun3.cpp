...
void Graph::WriteLabels(string file){
    ...
    for(int ID1=0;ID1<nodenum;ID1++){
        if(!PruningPoint[ID1].empty()){
            for(auto it=PruningPoint[ID1].begin();it!=PruningPoint[ID2].end();++it){
                ID2=it->first;
                if(!it->second.empty()){
                    OF2<<ID1<<" "<<ID2;
...
//    Construct_core();//index construction
    ReadLabels(graphfile+".label");
...
//    for(int u=0;u<testdata2.size();++u){
    for(int u=0;u<1;++u){
        ...
        cout<<"Before update: "<<QueryCore(261222,146486)<<" "<<DijkstraCore(261222,146486)<<endl;
        if(Label[261222].find(146447) != Label[261222].end()){//if found
            cout<<Label[261222][146447]<<" "<<QueryCore(261222,146447)<<" "<<DijkstraCore(261222,146447)<<endl;
        }

//        DijkstraPathCore(261222,146486);
//                g.QueryCoreDebug(142488,143850);
        t10=std::chrono::high_resolution_clock::now();
//        g.IncreasePSL(ID1,ID2,oldW,newW,g.AdjaCore,g.Label,g.PruningPointNew,g.NoSupportedPair);
        IncreasePSL(ID1,ID2,oldW,newW,AdjaCore,Label,PruningPoint);
...
//        g.CorrectnessCheck(200);
        cout<<"After update: "<<QueryCore(261222,146486)<<" "<<DijkstraCore(261222,146486)<<endl;
        if(Label[261222].find(146447) != Label[261222].end()){//if found
            cout<<Label[261222][146447]<<" "<<QueryCore(261222,146447)<<" "<<DijkstraCore(261222,146447)<<endl;
        }
        if(u==3){// || u==279
//            WriteCoreGraph(graphfile+"Core2");
//            WriteLabels(graphfile+".label");
//            cout<<"Done."<<endl;
