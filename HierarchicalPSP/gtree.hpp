...
// gtree index disk storage
    FILE_NODES_GTREE_PATH = dataset + "." + to_string(LEAF_CAP) + ".paths.bin";
    FILE_GTREE = dataset + "." + to_string(LEAF_CAP) +".gtree.bin";
    FILE_ONTREE_MIND = dataset + "." + to_string(LEAF_CAP) + ".minds.bin";
// input
    FILE_OBJECT = dataset + "." + to_string(LEAF_CAP) + ".object";
    FILE_SHORTCUT = dataset + "." + to_string(LEAF_CAP)+ ".shortcuts";
...
//    FILE *fout = fopen( filename, "wb" );
    string ofilename=DataPath+"/"+dataset+"/tmp/"+dataset+".gtree";
    ofstream ofile(ofilename);
    if(!ofile){
        cout<<"Failed to open file "<<ofilename<<endl;
        exit(1);
    }
    ...
    for ( int i = 0; i < GTree.size(); i++ ){
        // father
//        fwrite( &GTree[i].father, sizeof(int), 1, fout );
        ofile<<i<<" "<<GTree[i].father;
        // isleaf
//        fwrite( &GTree[i].isleaf, sizeof(bool), 1, fout );
        ofile<<" "<<GTree[i].isleaf;
        // children
//        int count_children = GTree[i].children.size();
//        fwrite( &count_children, sizeof(int), 1, fout );
//        ofile<<" "<<count_children;
////        copy( GTree[i].children.begin(), GTree[i].children.end(), buf );
////        fwrite( buf, sizeof(int), count_children, fout );
//        if(count_children>0){
//            for(auto it1=GTree[i].children.begin();it1!=GTree[i].children.end();++it1){
//                ofile<<" "<<*it1;
//            }
//        }
        // borders
...
//        fwrite( buf, sizeof(int), count_borders, fout );
        if(count_borders>0){
            for(auto it1=GTree[i].borders.begin();it1!=GTree[i].borders.end();++it1){
                ofile<<" "<<*it1;
            }
        }
        // leafnodes
        int count_leafnodes = GTree[i].leafnodes.size();
//        fwrite( &count_leafnodes, sizeof(int), 1, fout );
        ofile<<" "<<count_leafnodes;
//        copy( GTree[i].leafnodes.begin(), GTree[i].leafnodes.end(), buf );
//        fwrite( buf, sizeof(int), count_leafnodes, fout );
        if(count_leafnodes>0){
            for(auto it1=GTree[i].leafnodes.begin();it1!=GTree[i].leafnodes.end();++it1){
                ofile<<" "<<*it1;
            }
        }
...
// original dijkstra search, used for single-source shortest path search WITHIN one gtree leaf node!//old version
// input: s = source node
...
//    q[s] = 0;//map of source vertex
    ...
    while( ! todo.empty() && ! q.empty() ){
        ...
        q.pop();
        // put min to result, add to visited
        while(closed.find(minpos)!=closed.end()){//if found
            if(!q.empty()){
                topE=q.top();
                min=topE.first; minpos=topE.second;
                q.pop();
        ...
        if(closed.find(minpos)==closed.end()){//if not found
            result[minpos] = min;
            closed.insert( minpos );
        }else{
